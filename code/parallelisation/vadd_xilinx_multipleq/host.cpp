...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < num_cu; i++) {
        ...
        OCL_CHECK(err, err = q1.enqueueMigrateMemObjects({buffer_in1[i], buffer_in2[i]}, 0 /* 0 means from host*/));
        OCL_CHECK(err, err = q2.enqueueMigrateMemObjects({buffer_in1[i], buffer_in2[i]}, 0 /* 0 means from host*/));
        OCL_CHECK(err, err = q3.enqueueMigrateMemObjects({buffer_in1[i], buffer_in2[i]}, 0 /* 0 means from host*/));
    }
    ...
    // Launch the kernel
    for(int i=0; i<num_cu;i++){
        OCL_CHECK(err, err = q1.enqueueTask(krnls[i]));
        OCL_CHECK(err, err = q2.enqueueTask(krnls[i]));
        OCL_CHECK(err, err = q3.enqueueTask(krnls[i]));
    }
    ...
    for (int i = 0; i < num_cu; i++) {
        OCL_CHECK(err, err = q1.enqueueMigrateMemObjects({buffer_output[i]}, CL_MIGRATE_MEM_OBJECT_HOST));
        OCL_CHECK(err, err = q2.enqueueMigrateMemObjects({buffer_output[i]}, CL_MIGRATE_MEM_OBJECT_HOST));
        OCL_CHECK(err, err = q3.enqueueMigrateMemObjects({buffer_output[i]}, CL_MIGRATE_MEM_OBJECT_HOST));
    }
    OCL_CHECK(err, err = q1.finish());
    OCL_CHECK(err, err = q2.finish());
    OCL_CHECK(err, err = q3.finish());
